...
extension SearchViewController: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        navigationController?.pushViewController(RestroDetailsController.control(with: items[indexPath.row]), animated: true)
    }
}
...
extension SearchViewController: UISearchBarDelegate {
    ...
    func searchBarCancelButtonClicked(_ searchBar: UISearchBar) {
        indicator.startAnimating()
        YelpManager.fetchYelpBusinesses(with: offset, location: self.currentLocationName) { (baseModel) in
            self.indicator.stopAnimating()
            self.items.removeAll()
            self.baseModel = baseModel
        }
